...
class GameScreen:
    def __init__(self, playerName, servPort, servIP):
        ...
        self.currentMenu.add_command(label = "New Game!", command = self.newGame)
    ...
    def counter_label(self):
        if self.running:
            ...
            if(self.counter == -1):
                self.running = False
                self.resetTimer()
                if(self.paintModeToggle.get() == 1 and self.gameOngoing == True):
                    print("Im here")
                    self.sendMsg = "NP//"
                    self.sendMsgNow()
...
class ShowStartScreen():
    def __init__(self):
        ...
        self.defaultIP = StringVar(self.startScreen, value='192.168.1.2')
