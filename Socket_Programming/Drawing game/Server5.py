...
def msgInterpreter(data,sock):
    ...
    if len(msgParts) == 2: # All message types, except for Word Guesses, are made out of 2 elements.
        ...
        if msgType == "NG":
            print(f"New Game requested from player {playerNames[sock.fileno()]}")
            playerFDList = list(playerNames)
            currentplayer = sock.fileno()
            theWord = dealer.newWord()
            playerSettings = str(msgValue)
            data = str("NW:"+str(playerNames[sock.fileno()])+":"+str(theWord))
            print(data)
        ...
        if msgType == "NP":
            print("Got it")
            if(len(playerFDList)-1 == playerFDList.index(currentplayer)):
                currentplayer = playerFDList[0]
            else:
                currentplayer = playerFDList[playerFDList.index(currentplayer)+1]
            data = "-1"
            for s in connections:
                if(s.fileno() == currentplayer):
                    temp = "YT: Your Turn :Do it"
                    temp.encode()
                    s.send(temp) 
